...
func init() {
	routerCheckRole = append(routerCheckRole, registerCostCenterInfoChangeRouter)
}

// registerCostCenterInfoChangeRouter
func registerCostCenterInfoChangeRouter(v1 *gin.RouterGroup, authMiddleware *jwt.GinJWTMiddleware) {
	api := apis.CostCenterInfoChange{}
	r := v1.Group("/cost-center-info-change").Use(authMiddleware.MiddlewareFunc()).Use(middleware.AuthCheckRole())
	{
