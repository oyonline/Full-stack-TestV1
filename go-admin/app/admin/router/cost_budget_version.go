...
func init() {
	routerCheckRole = append(routerCheckRole, registerCostBudgetVersionRouter)
}

// registerCostBudgetVersionRouter
func registerCostBudgetVersionRouter(v1 *gin.RouterGroup, authMiddleware *jwt.GinJWTMiddleware) {
	api := apis.CostBudgetVersion{}
	r := v1.Group("/cost-budget-version").Use(authMiddleware.MiddlewareFunc()).Use(middleware.AuthCheckRole())
	{
