...
func init() {
	routerCheckRole = append(routerCheckRole, registerCostCenterInfoRouter)
}

// registerCostCenterInfoRouter
func registerCostCenterInfoRouter(v1 *gin.RouterGroup, authMiddleware *jwt.GinJWTMiddleware) {
	api := apis.CostCenterInfo{}
	r := v1.Group("/cost-center-info").Use(authMiddleware.MiddlewareFunc()).Use(middleware.AuthCheckRole())
	{
		...
		//r.GET("/dropdownData", api.DropdownData)
	}
}
